...
def index():
    if request.method == 'POST':
        ...
        pred_mac, pred_prob, claim_prob, rwf = predictor.predict(svc, svc.config['Model'], svc.config['Le'], STAGE, claim_mac)
...
def update(rwf, claim_mac, rwfs, macs):
    ...
    # Rebuild
    model, le = builder.build(svc, svc.config['Rwfs'], svc.config['Macs'])
    return model, le


def setup():
    svc.config['Rwfs'], svc.config['Macs'] = reader.read(svc, DATASET)

    if os.path.exists(MODEL) and os.path.exists(LABELS):
        svc.config['Model'] = load_model(MODEL)
        svc.config['Le'] = joblib.load(LABELS)
    else:
        svc.config['Model'], svc.config['Le'] = builder.build(svc, svc.config['Rwfs'], svc.config['Macs'])
